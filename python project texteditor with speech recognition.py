# -*- coding: utf-8 -*-
"""
Created on Thu Nov  5 00:38:26 2020
...
font_size["values"]=tuple(range(8,80,3))                       ## here also we are passing in which some values are there  ## we have given a range from which user can select the size and also we can give difference between two values 
font_size.current(font_size.index(30))                           ## default value of size
...
#mic button
mic_btn=ttk.Button(toolbar_label,text="speak")
mic_btn.grid(row=0,column=6,padx=5)

#align left,right,center
alignleft_btn=ttk.Button(toolbar_label,text="left")
alignleft_btn.grid(row=0,column=7,padx=5)
aligncenter_btn=ttk.Button(toolbar_label,text="center")
aligncenter_btn.grid(row=0,column=8,padx=5)
alignright_btn=ttk.Button(toolbar_label,text="right")
alignright_btn.grid(row=0,column=9,padx=5)                     ##whenever we know that we to fi something at the top, bottom, left, right then we are using pack and when we want to fix something using row and column then we are using grid
...
current_font_size=11                                           ## these are the golbal variables
...
def mic_func():
    ...
    msg=ttk.Label()
    msg.pack()
    btn = ttk.Button(text="click here to start",command=rec)
    btn.pack()
mic_btn.configure(command=mic_func)
...
file.add_command(label="Status Bar",compound=tkinter.LEFT,command=txt_changed)   
